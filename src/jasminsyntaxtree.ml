...
let parse_and_print arch call_conv =
  ...
  fun output file ->
    ...
    let out, close =
      match output with
      | None -> (stdout, ignore)
      | Some dotfile -> (open_out dotfile, close_out)
    in
    let fmt = Format.formatter_of_out_channel out in
    Format.fprintf fmt "%a@." Dot_printer.pp_prog ast;
    close out
...
let output =
  ...
  in
  Arg.(value & opt (some string) None & info [ "o"; "output" ] ~docv:"DOT" ~doc)

let () =
  let doc = "Pretty-print Jasmin source programs into Graphviz representation" in
  let man =
    [
  ...
  in
  let info =
    Cmd.info "jasmin-syntaxtree" ~version:Glob_options.version_string ~doc ~man
  in
